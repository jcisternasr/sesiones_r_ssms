...
le_hblt <- data_procesada %>%
  filter(abrev_destino == 'HBLT', is.na(c_salida)) %>% 
  mutate(
    edad_hoy = round(interval(fecha_nac,today())/years(1)),
    tiempo_espera = interval(f_entrada,today())/days()) %>% 
  group_by(desc_especialidad,nivel_de_atencion) %>% 
  summarise(mean_t = round(mean(tiempo_espera),1)) %>%
  slice_max(order_by = mean_t, n=10)

le_hblt <- le_hblt
  mutate(desc_especialidad = reorder(desc_especialidad, mean_t, .desc = TRUE))

ggplot(le_hblt, aes(x = desc_especialidad, y = mean_t)) +
  geom_bar(stat = "identity") +
  theme_test()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Especialidad", y = "Tiempo", title = "Tiempo por Especialidad") +
